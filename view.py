...
def update_customer_details(customer_id):
    ...
    if identity.get('role') == 'manager' and str(customer.get('manager_id')) != str(identity.get('user_id')):
        return jsonify(ok=False, message="You can only edit customers assigned to your management."), 403
    try:
        audited_details_update(db, customers_collection, customer, updates, identity,
                               'customer.details_changed', customer, history=history_doc)
    except (PyMongoError, ValueError):
        return jsonify(ok=False, message="Unable to save changes. Reload the customer and try again."), 409
...
def update_customer_location(customer_id):
    ...
    try:
        ...
        if not customer:
            return jsonify(error="Customer not found"), 404
        if identity.get('role') == 'manager' and str(customer.get('manager_id')) != str(identity.get('user_id')):
            return jsonify(error="Customer outside your access"), 403
        if not (-90 <= lat <= 90 and -180 <= lon <= 180):
            return jsonify(error="Invalid coordinates"), 400
        coordinates = {**(customer.get('coordinates') or {}), 'latitude': lat, 'longitude': lon}
        audited_details_update(db, customers_collection, customer, {'coordinates': coordinates},
                               identity, 'customer.details_changed', customer)
