...
def sync_loan(loan):
    ...
    result = loans_col.update_one({"_id": loan["_id"], "status": {"$in": ["active", "approved", "grace_period", "overdue"]}}, {"$set": {k: loan[k] for k in ("penalty_cycles_applied", "total_penalties", "current_balance", "next_penalty_date", "status", "updated_at")}})
    if result.matched_count == 0:
        return loans_col.find_one({"_id": loan["_id"]}) or loan
    return loan
...
def index():
    ...
    for loan in loans_col.find(query).sort("created_at", -1):
        ...
        if not text or text in f"{customer.get('name','')} {customer.get('phone_number','')} {row.get('loan_number','')}".lower():
            docs.append(row)
    counts = {s: loans_col.count_documents({**query, "status": s}) for s in ("pending", "active", "grace_period", "overdue", "settled", "cancelled")}
...
def executive_index():
    ...
    totals = {"count": len(rows), "principal": sum(r["original_amount"] for r in rows),
              "paid": sum(r["amount_paid"] for r in rows), "balance": sum(r["current_balance"] for r in rows if r.get("status") != "cancelled")}
...
def cancel(loan_id):
    ...
    if not token or not secrets.compare_digest(token, session.get("loan_cancel_token", "")):
        abort(403)
    reason = request.form.get("reason", "").strip()
    if not reason or len(reason) > 500:
        flash("Enter a cancellation reason of up to 500 characters.", "danger")
        return redirect(url_for("loans.detail", loan_id=loan_id))
    cancellable = ["approved", "active", "grace_period", "overdue"]
    if loan.get("status") not in cancellable:
        flash("Only approved, ongoing loans can be cancelled.", "warning")
        return redirect(url_for("loans.detail", loan_id=loan_id))
    payment_count = max(
        payments_col.count_documents({"loan_id": loan["_id"], "payment_type": "LOAN", "amount": {"$gt": 0}}),
        int(loan.get("loan_payment_count") or 0),
    )
    if payment_count >= 3:
        flash("This loan cannot be cancelled because it has three or more payments.", "warning")
        return redirect(url_for("loans.detail", loan_id=loan_id))
    now = datetime.utcnow()
    result = loans_col.update_one(
        {"_id": loan["_id"], "status": {"$in": cancellable},
         "amount_paid": loan.get("amount_paid"),
         "$or": [{"loan_payment_count": {"$lt": 3}}, {"loan_payment_count": {"$exists": False}}]},
        {"$set": {"status": "cancelled", "cancellation_reason": reason,
                  "cancelled_at": now, "cancelled_by": str(session.get("executive_id") if role == "executive" else session.get("manager_id")),
                  "cancelled_by_role": role, "updated_at": now}},
    )
    if result.modified_count == 1:
        flash("Loan cancelled successfully. The agent can now see it as cancelled.", "success")
    else:
        flash("The loan changed before cancellation. Please review its current status.", "warning")
    return redirect(url_for("loans.detail", loan_id=loan_id))


@loans_bp.post("/<loan_id>/guarantor")
def update_guarantor(loan_id):
    if _role() != "agent":
        return jsonify(ok=False, error="Only agents can edit guarantor details."), 403
    loan = _owned_loan(loan_id)
    if not loan:
        abort(404)
    token = request.form.get("guarantor_token", "")
    if not token or not secrets.compare_digest(token, session.get("guarantor_edit_token", "")):
        return jsonify(ok=False, error="Your session expired. Reload the page and try again."), 403
    names = request.form.getlist("guarantor_name[]")
    values = request.form.getlist("guarantor_value[]")
    if len(names) != len(values) or len(names) > 50:
        return jsonify(ok=False, error="Supply matching detail names and values (up to 50 details)."), 400
    if any(len(name.strip()) > 100 for name in names) or any(len(value.strip()) > 500 for value in values):
        return jsonify(ok=False, error="Detail names must be at most 100 characters and values at most 500."), 400
    try:
        rows = _detail_rows("guarantor")
    except ValueError as exc:
        return jsonify(ok=False, error=str(exc)), 400
    if not rows:
        return jsonify(ok=False, error="Add at least one guarantor detail before saving."), 400
    from login import get_current_identity
    from services.change_activities import audited_details_update
    from pymongo.errors import PyMongoError
    customer = customers_col.find_one({'_id': loan.get('customer_id')}) or {'_id': loan.get('customer_id'), 'name': loan.get('customer_name', '')}
    try:
        audited_details_update(db, loans_col, loan, {'guarantor_details': rows}, get_current_identity(),
                               'loan.guarantor_changed', customer, metadata={'guarantor_updated_at': datetime.utcnow(), 'guarantor_updated_by': str(current_user.id)})
    except (PyMongoError, ValueError):
        return jsonify(ok=False, error="Unable to save guarantor changes. Reload and try again."), 409
...
def detail(loan_id):
    ...
    if _role() in {"manager", "executive"} and not session.get("loan_cancel_token"):
        session["loan_cancel_token"] = secrets.token_urlsafe(32)
    if _role() == "agent" and not session.get("guarantor_edit_token"):
        session["guarantor_edit_token"] = secrets.token_urlsafe(32)
    loan = sync_loan(loan)
    ...
    if start and end and loan.get("status") != "cancelled":
        cursor = start
        while cursor <= end:
            if cursor.weekday() != 6:
    ...
    return render_template("loans/detail.html", loan=display(loan), customer=customer, payments=payments,
                           ...
                           cancellation_payment_count=max(sum(1 for p in payments if money(p.get("amount")) > 0), int(loan.get("loan_payment_count") or 0)),
