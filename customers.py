...
def edit_customer(customer_id):
    ...
    customer = customers_col.find_one({'_id': ObjectId(customer_id), 'manager_id': {'$in': [str(identity.get('user_id')), ObjectId(session['manager_id'])]}})
    if not customer:
        return "Customer not found or outside your access", 404
    try:
        changed = audited_details_update(db, customers_col, customer, form_data, identity, 'customer.details_changed', customer)
    except (PyMongoError, ValueError):
        flash("Unable to save customer changes. Reload and try again.", "danger")
    else:
        flash("Customer updated successfully!" if changed else "No changes made.", "success")
    return redirect(url_for("customers.customer_profile", customer_id=customer_id))
...
def delete_customer(customer_id):
    ...
    if not identity.get("is_authenticated") or role not in {"manager", "executive"}:
        return jsonify(ok=False, message="Manager or executive access required."), 403

    endpoint = ("executive_customers.executive_customers_list" if role == "executive"
                else "customers.customers_list")

    def respond(message, status=200):
        if request.accept_mimetypes.best == "application/json":
            return jsonify(ok=status == 200, message=message), status
        flash(message, "success" if status == 200 else "danger")
        return redirect(url_for(endpoint))

    if not ObjectId.is_valid(customer_id):
        return respond("Invalid customer ID.", 400)
    query = {"_id": {"$in": [ObjectId(customer_id), customer_id]}}
    if role == "manager":
        manager_id = str(identity.get("user_id") or "")
        variants = [manager_id]
        if ObjectId.is_valid(manager_id):
            variants.append(ObjectId(manager_id))
        query["manager_id"] = {"$in": variants}

    def move_customer(mongo_session):
        customer = customers_col.find_one(query, session=mongo_session)
        if not customer:
            return False
        # Preserve the complete document, including its original BSON _id.
        # The transaction rolls back both writes if either operation fails.
        deleted_col.insert_one(customer, session=mongo_session)
        record_change(db, identity, 'customer.deleted', customer,
                      {'collection': {'from': 'customers', 'to': 'deleted'}}, mongo_session=mongo_session)
        result = customers_col.delete_one({"_id": customer["_id"]}, session=mongo_session)
        if result.deleted_count != 1:
            raise RuntimeError("Customer changed during deletion")
        return True

    try:
        with db.client.start_session() as mongo_session:
            moved = mongo_session.with_transaction(move_customer)
    except (PyMongoError, RuntimeError):
        return respond("Customer could not be moved to Deleted Customers. Please try again.", 503)
    if not moved:
        return respond("Customer not found or outside your access.", 404)
    return respond("Customer moved to Deleted Customers. Their ID, details and payments are preserved.")


def _deleted_customer_scope():
    from login import get_current_identity

    identity = get_current_identity()
    if not identity.get("is_authenticated") or identity.get("role") not in {"manager", "executive"}:
        return None
    ...
    if ObjectId.is_valid(manager_id):
        variants.append(ObjectId(manager_id))
    return {"manager_id": {"$in": variants}}


@customers_bp.get("/customers/deleted")
def deleted_customers_list():
    query = _deleted_customer_scope()
    if query is None:
        return jsonify(ok=False, message="Manager or executive access required."), 403
    ...
    per_page = 20
    total = deleted_col.count_documents(query)
    pages = max(1, (total + per_page - 1) // per_page)
    page = min(page, pages)
    rows = deleted_col.find(query, {"name": 1, "phone_number": 1}).sort("_id", -1).skip((page - 1) * per_page).limit(per_page)
    return jsonify(ok=True, customers=[{"id": str(row["_id"]), "name": row.get("name") or "Unnamed customer",
                                        "phone": row.get("phone_number") or "N/A"} for row in rows],
                   page=page, pages=pages, total=total)


@customers_bp.post("/customer/<customer_id>/restore")
def restore_deleted_customer(customer_id):
    ...
    def restore(mongo_session):
        customer = deleted_col.find_one(query, session=mongo_session)
        if not customer:
            return False
        customers_col.insert_one(customer, session=mongo_session)
        record_change(db, identity, 'customer.restored', customer,
                      {'collection': {'from': 'deleted', 'to': 'customers'}}, mongo_session=mongo_session)
        result = deleted_col.delete_one({"_id": customer["_id"]}, session=mongo_session)
        if result.deleted_count != 1:
            raise RuntimeError("Deleted customer changed during restoration")
        return True

    try:
        with db.client.start_session() as mongo_session:
            restored = mongo_session.with_transaction(restore)
    except DuplicateKeyError:
        return jsonify(ok=False, message="A customer with this ID already exists. No records were overwritten."), 409
    except (PyMongoError, RuntimeError):
        return jsonify(ok=False, message="Unable to restore customer. Please try again."), 503
    if not restored:
        return jsonify(ok=False, message="Deleted customer not found or outside your access."), 404
    return jsonify(ok=True, message="Customer restored with their original ID, details and payment links.")
