...
export function AddInventoryItemModal({
  ...
  // Text inputs prevent wheel/arrow changes; validate the complete price before saving.
  const isValidPrice = (value: string) => /^(?:[0-9]+(?:\.[0-9]*)?|\.[0-9]+)$/.test(value.trim()) && Number.isFinite(Number(value));
  ...
  const validate = (): boolean => {
    ...
    if (!isValidPrice(costPrice) || cost <= 0) newErrors.costPrice = 'Cost price must be greater than 0.';
    if (!isValidPrice(sellingPrice) || sell <= 0) newErrors.sellingPrice = 'Selling price must be greater than 0.';
    if (installmentPrice && !isValidPrice(installmentPrice)) newErrors.installmentPrice = 'Enter a valid non-negative installment price.';
    if (wholesalePrice && !isValidPrice(wholesalePrice)) newErrors.wholesalePrice = 'Enter a valid non-negative wholesale price.';
    if (!category) newErrors.category = 'Please select a category.';
    if (!isEditMode && selectedBranches.length === 0) newErrors.branches = 'Select at least one branch.';
    const missingLocationBranch = selectedBranches.find((branch) => !selectedLocations[branch]);
    if (!isEditMode && missingLocationBranch) newErrors.locations = `Select a warehouse or location for ${missingLocationBranch}.`;
  ...
  const canSubmit = !!uploadedImageUrl && !!productName.trim() && !!costPrice && !!sellingPrice && (isEditMode || (selectedBranches.length > 0 && selectedBranches.every((branch) => Boolean(selectedLocations[branch])))) && !isSubmitting && !isUploadingImage;
  ...
  return (
    <ProductEditorDialog title={isEditMode ? 'Edit Inventory Item' : 'Add Inventory Item'} busy={isSubmitting || isUploadingImage} onClose={onClose}>
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-4 shrink-0 border-b border-gray-100 bg-white sticky top-0 z-10">
          ...
          <button
            ...
            className="shrink-0 p-3 hover:bg-gray-100 rounded-lg transition-colors text-gray-500 hover:text-gray-700"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Step progress */}
        <div className="flex shrink-0 flex-wrap items-center gap-y-2 px-4 sm:px-6 py-3 bg-gray-50 border-b border-gray-100 text-xs">
          {[
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {showAdvancedPricing && (
                    <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">
                        ...
                        {errors.installmentPrice && <p className="text-xs text-red-500 mt-1">{errors.installmentPrice}</p>}
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">
                          Wholesale Price (GHS)
                        ...
                        {errors.wholesalePrice && <p className="text-xs text-red-500 mt-1">{errors.wholesalePrice}</p>}
                      </div>
                    </div>
                  )}
                </div>
        ...
        {/* Sticky Footer */}
        <div className="px-4 sm:px-6 py-4 shrink-0 border-t border-gray-100 bg-white flex flex-col sm:flex-row sm:items-center justify-between gap-3">
