...
export function ProductCards() {
  ...
  return (
    <div className="space-y-6">
      {editingCard && (
        <CreateProductCardModal bootstrap={bootstrap} card={editingCard}
          onClose={() => setEditingCard(null)}
          onCreated={async () => { setEditingCard(null); await loadProductCards(true); }} />
      )}
      {createOpen && (
        <CreateProductCardModal
          bootstrap={bootstrap}
              ...
              {paginatedCards.map((card) => (
                <div key={card.id} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
                  ...
                  <div className="space-y-4 p-5">
                    ...
                    <button type="button" onClick={() => setEditingCard(card)} aria-label={`Edit ${card.name}`} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-2xl border border-indigo-200 px-4 py-3 text-sm font-medium text-indigo-700 hover:bg-indigo-50"><Edit className="h-4 w-4" /> Edit</button>
                    <button
                ...
                {paginatedCards.map((card) => (
                  <tr key={card.id} className="transition hover:bg-slate-50">
                    ...
                    <td className="px-6 py-5 text-right">
                      <button type="button" onClick={() => setEditingCard(card)} aria-label={`Edit ${card.name}`} className="inline-flex min-h-11 items-center gap-2 rounded-2xl border border-indigo-200 px-4 py-2 text-sm font-medium text-indigo-700 hover:bg-indigo-50"><Edit className="h-4 w-4" /> Edit</button>
                      <button
                        onClick={() => setSelectedCard(card)}
...
function CreateProductCardModal({
  ...
  return (
    <ProductEditorDialog title={isEditing ? 'Edit Product Card' : 'Create Product Card'} busy={submitting} onClose={onClose} wide>
        <div className="flex items-start justify-between shrink-0 gap-3 border-b border-slate-200 px-4 sm:px-6 py-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">{isEditing ? 'Edit Product Card' : 'Create Product Card'}</h3>
          ...
          <button
            onClick={onClose}
                disabled={submitting}
            aria-label="Close product card editor"
            className="inline-flex shrink-0 h-11 w-11 items-center justify-center rounded-2xl border border-slate-200 text-slate-500 transition hover:bg-slate-50"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
                ...
                <div className="mt-4 max-h-72 space-y-3 overflow-y-auto pr-1">
                  {filteredInventory.map((item) => {
                    ...
                    return (
                      <div
                        key={item.key}
                        className={`flex flex-wrap sm:flex-nowrap items-center gap-3 rounded-2xl border p-3 transition ${
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={submit}
                disabled={submitting}
                className="inline-flex flex-1 sm:flex-none items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-4 py-3 text-sm font-medium text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-70"
              >
